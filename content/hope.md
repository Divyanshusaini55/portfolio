...
#### from the paper, Nested Learning: The Illusion of Deep Learning Architecture by: [Ali Behrouz, Meisam Razaviyayn, Peilin Zhong, and Vahab Mirrokni](http://abehrouz.github.io/files/NL.pdf) — Google Research, NeurIPS 2025
---
...
$$
\tilde{\mathcal{L}} = \|\mathcal{M}\mathbf{K} - \mathbf{V}\|^2_2
$$
...
But there's a crucial difference, in linear attention, keys and values are *independent of the memory state* $\mathcal{M}$. You can pre-compute them. But here the value i.e., the gradient **depends on the current state** $W_t$.
...
At each step, $\mathbf{v}_t$ is *generated by the memory* $W_t$ itself. The memory decides what to learn next based on where it currently is. This is much more powerful than simple linear attention, where you're just mapping pre-given keys to pre-given values.
...
Each $\text{MLP}^{(f_\ell)}$ has its own chunk size $C^{(\ell)}$ that determines its update frequency. Here's the crucial update rule:

$$
\theta^{(f_\ell)}_{i+1} = \begin{cases}
\theta^{(f_\ell)}_i - \sum_{t=i-C^{(\ell)}}^{i} \eta^{(\ell)}_t f(\theta^{(f_\ell)}_t; \mathbf{x}_t) & \text{if } i \equiv 0 \pmod{C^{(\ell)}} \\
0 & \text{otherwise}
\end{cases}
$$
...
Let me decode what this is saying. The parameters $\theta^{(f_\ell)}$ at level $\ell$ only update when $i$ is divisible by the chunk size $C^{(\ell)}$. When that happens, they accumulate all the gradients (or more generally, the error signal $f(\theta^{(f_\ell)}_t; \mathbf{x}_t)$) from the past $C^{(\ell)}$ tokens and apply them all at once. Otherwise, the update is zero, the parameters freeze.
...
The frequency $f_\ell = \frac{L_{\text{max}}}{C^{(\ell)}}$ determines how many times a level updates over the maximum sequence length. High frequency means many updates (local, adaptive). Low frequency means few updates (global, persistent). And by stacking multiple levels, each with its own $C^{(\ell)}$, you create a spectrum which is the **Continuum Memory System**.
...
$$
W_{t+1} = \arg\min_W \langle W\mathbf{x}_t, \mathbf{u}_t \rangle + \frac{1}{2\eta_t}\|W - W_t\|^2_2
$$
...
$$
W_{t+1} = \arg\min_W \frac{1}{2}\|W\mathbf{x}_t - \mathbf{u}_t\|^2_2 + \frac{1}{2\eta_t}\|W - W_t\|^2_2
$$
...
where $\beta_t = \frac{1}{\eta_t} - \frac{\lambda^2}{\eta_t(\lambda^2 + \eta_t)}$.

Look at that first term: $W_t(I - \alpha_t\mathbf{x}_t\mathbf{x}_t^\top)$ where $\alpha_t = \frac{1}{\lambda^2+\eta_t}$.
...
Fixed projection matrices $W_k, W_v, W_q$ from pre-training. but we want them to adapt
...
- Learning rate $\eta_t$? Generated by the model
- Decay factor $\alpha_t$? Generated by the model  
...
$$
\eta_t = \mathcal{M}_{\eta,C \times \lceil t/C \rceil}(\mathbf{x}_t), \quad \alpha_t = \mathcal{M}_{\alpha,C \times \lceil t/C \rceil}(\mathbf{x}_t)
$$
...
$$
\mathbf{m}^{(1)}_{t+1} = \beta_1 \mathbf{m}^{(1)}_t + (1-\beta_1)\nabla_W\mathcal{L}
$$
...
\beta_K \mathbf{m}^{(K)}_i + (1-\beta_K) \sum_{j} \nabla_W\mathcal{L}_j & \text{if } i \equiv 0 \pmod{K} \\
